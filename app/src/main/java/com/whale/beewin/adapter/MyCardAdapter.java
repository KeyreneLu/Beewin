...
public class MyCardAdapter extends BaseAdapter {
    ...
    private List<MyCard.MlistBean> datas;
    private ViewHolder mHolder;
    int mSelect = -1;
    public MyCardAdapter(Context context, List<MyCard.MlistBean> datas) {
        mContext = context;
        this.datas = datas;
    }
    @Override
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        MyCard.MlistBean bean = datas.get(position);
        if (convertView == null) {
        ...
        mHolder.mRlCardPic.setBackgroundResource(R.drawable.xjj_orange);
        mHolder.mTvTime.setText(bean.getDays());
        mHolder.mCashMoney.setText(bean.getScore()+"元");
            mHolder.mTvDeadLine.setText(bean.getYxq());
        if (mSelect == position){
