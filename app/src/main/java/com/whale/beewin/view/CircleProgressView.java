...
public class CircleProgressView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setColor(getResources().getColor(R.color.touzi_fenshu));
        canvas.drawArc(mRectF, -90, ((float) mProgress / mMaxProgress) * 360, false, mPaint);
        // 绘制进度文案显示
        mPaint.setStrokeWidth(mTxtStrokeWidth);
        String text1 = title;
        int textHeight1 = height / 4;
        mPaint.setTextSize((float) (textHeight1 / 1.4));
        int textWidth1 = (int) mPaint.measureText(text1, 0, text1.length());
        mPaint.setStyle(Style.FILL);
        canvas.drawText(text1, width / 2 - textWidth1 / 2, height / 2 - 10 + textHeight1 / 2 - 10, mPaint);
        // 绘制进度文案显示
        mPaint.setStrokeWidth(mTxtStrokeWidth);
        String text = mProgress + "%";
        int textHeight = height / 4;
        mPaint.setTextSize((float) (textHeight / 1.4));
        int textWidth = (int) mPaint.measureText(text, 0, text.length());
        mPaint.setStyle(Style.FILL);
        canvas.drawText(text, width / 2 - textWidth / 2, height / 2 + 10 + textHeight / 2 + 10, mPaint);

        if (!TextUtils.isEmpty(mTxtHint1)) {
            mPaint.setStrokeWidth(mTxtStrokeWidth);
            text = mTxtHint1;
            textHeight = height / 8;
            mPaint.setTextSize(height / 12);
